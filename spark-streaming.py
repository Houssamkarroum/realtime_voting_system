...
print(pyspark.__version__)  # Check the version of PySpark
...
if __name__ == "__main__":
    ...
    spark = (SparkSession.builder
             ...
             .config("spark.jars",
                     "/home/legend/PycharmProjects/VotingSystemEngineering/postgresql-42.7.5.jar")  # PostgreSQL driver
    ...
    votes_df = kafka_stream.selectExpr("CAST(value AS STRING)").select(from_json(col("value"), vote_schema).alias("data")).select("data.*")
    ...
    votes_per_candidate_to_kafka = votes_per_candidate.selectExpr("to_json(struct(*)) AS value") \
        ...
        .option("checkpointLocation", "hdfs://localhost:8020/user/legend/checkpoints/checkpoint1") \
        .outputMode("update") \
        .start()
    ...
    turnout_by_location_to_kafka = turnout_by_location.selectExpr("to_json(struct(*)) AS value") \
        ...
        .option("checkpointLocation", "hdfs://localhost:8020/user/legend/checkpoints/checkpoint2") \
        .outputMode("update") \
        .start()
